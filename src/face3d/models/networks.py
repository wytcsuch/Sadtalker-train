    ...
    def __init__(self, net_recon, use_last_fc=False, init_path=None):
        ...
        if not use_last_fc:
            self.final_layers = nn.ModuleList([
                conv1x1(last_dim, 80, bias=True), # id layer  alpha
                conv1x1(last_dim, 64, bias=True), # exp layer  beta
                conv1x1(last_dim, 80, bias=True), # tex layer
                conv1x1(last_dim, 3, bias=True),  # angle layer  r
                conv1x1(last_dim, 27, bias=True), # gamma layer
                conv1x1(last_dim, 2, bias=True),  # tx, ty   
                conv1x1(last_dim, 1, bias=True)   # tz   (tx,ty,tz) = transalation
            ])
