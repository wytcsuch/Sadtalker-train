...
def split_coeff(coeffs):
        ...
        """
        id_coeffs = coeffs[:, :80]  #1 80
        exp_coeffs = coeffs[:, 80: 144]  #1 64
        tex_coeffs = coeffs[:, 144: 224]  #1 80
        angles = coeffs[:, 224: 227] #1  3
        gammas = coeffs[:, 227: 254] #1 27
        translations = coeffs[:, 254:] #1  3
        return {
            'id': id_coeffs,
            'exp': exp_coeffs,
            'tex': tex_coeffs,
...
class CropAndExtract():
    ...
    def generate_for_train(self, video_path, save_dir, org_dir, crop_or_resize='crop', source_image_flag=False, pic_size=256):
        video_name = os.path.splitext(os.path.split(video_path)[-1])[0] 
        dirname = os.path.dirname(video_path)
        save_path = dirname.replace(org_dir, save_dir) + '/'  + video_name  
        os.makedirs(save_path, exist_ok= True) 

        landmarks_path =  os.path.join(save_path, video_name + '_landmarks.txt')   #每一帧的图像landmark
        ...
        frames_pil = [Image.fromarray(cv2.resize(frame,(pic_size, pic_size))) for frame in x_full_frames]  #resize到256
        if len(frames_pil) == 0:
            print('No face is detected in the input file')
            return None, None
        ...
        for frame in frames_pil:
            ...
            cv2.imwrite(png_path, cv2.cvtColor(np.array(frame), cv2.COLOR_RGB2BGR))  #保存crop的图像
            i+= 1
    ...
    #检测landmark
    def detect_lm(self, input_path, save_dir, org_dir, crop_or_resize='crop', source_image_flag=False):
        pic_name = os.path.splitext(os.path.split(input_path)[-1])[0] 
        ...
        ####保存第一帧的landmarks
        if 'crop' in crop_or_resize.lower(): # default crop
            lm = self.propress.detect_lm(x_full_frames, still=True if 'ext' in crop_or_resize.lower() else False, xsize=512)
        savemat(landmarks_path,{'first_lm': lm})   
        

     #crop 256 and save
    def crop_256_save(self, input_path, save_dir, org_dir, crop_or_resize='crop', source_image_flag=False, pic_size = 256):
        # print(input_path)
        pic_name = os.path.splitext(os.path.split(input_path)[-1])[0] 
        dirname = os.path.dirname(input_path)
        save_path = dirname.replace(org_dir, save_dir) + '/'  + pic_name
        ...
        if 'crop' in crop_or_resize.lower(): # default crop
            x_full_frames, crop, quad = self.propress.align(x_full_frames, lm_68, still=True if 'ext' in crop_or_resize.lower() else False, xsize=512)
            clx, cly, crx, cry = crop
            lx, ly, rx, ry = quad
            lx, ly, rx, ry = int(lx), int(ly), int(rx), int(ry)
            oy1, oy2, ox1, ox2 = cly+ly, cly+ry, clx+lx, clx+rx
    ...
    def align_256_final(self, input_path, save_dir, org_dir, crop_or_resize='crop', source_image_flag=False, pic_size = 256):
        # print(input_path)
        pic_name = os.path.splitext(os.path.split(input_path)[-1])[0] 
        dirname = os.path.dirname(input_path)
        save_path = dirname.replace(org_dir, save_dir) + '/'  + pic_name
        ...
        landmarks_paths_256 = os.path.join(save_path, pic_name+'_landmarks.txt')   #256图像对应的landmarks
