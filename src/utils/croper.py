...
class Preprocesser:
    ...
    def crop(self, img_np_list, still=False, xsize=512):    # first frame for all video
        ...
        lm = self.get_landmark(img_np) #68个关键点
    ...
    ##仅仅检测landmark
    def detect_lm(self, img_np_list, still=False, xsize=512):    # first frame for all video
        img_np = img_np_list[0]
        lm = self.get_landmark(img_np) #68个关键点
        if lm is None:
            raise 'can not detect the landmark from source image'
    ...
    #align到256
    def align(self, img_np_list, lm, still=False, xsize=512):    # first frame for all video
        img_np = img_np_list[0]
        rsize, crop, quad = self.align_face(img=Image.fromarray(img_np), lm=lm, output_size=xsize)
        clx, cly, crx, cry = crop
        lx, ly, rx, ry = quad
